...
class ApplyLeaveController extends Controller {
    ...
    public function index()
    {
        $heading = "Leave";
        $title = "Apply Leave";
        $employee_leaves = $this->leave->query("SELECT ei.id,ei.EmployeeCode,EmployeeName,PunchCardNo,lta.id as leave_tran_id FROM employee_info ei 
    ...
    public function multiple_apply_leave()
    {
        ...
        if(isset($_POST['employee_id'])){
            ...
            return view('apply_leave/multiple_apply_leave',compact('title','heading'));
        }
    ...
    public function get_employee_info() {
        $search = $_GET['emp_id'];
        $emp_type = config('constants.employee_type');
        $emp_info = $this->leave->get_employee_info($search);
        $EmpType = '';
        if(!empty($emp_info['EmpType'])){
            $EmpType = $emp_type[$emp_info['EmpType']];
        }
        $leave_policy = $this->leave->table('leave_policy_master')->fetchAll();
        foreach ($leave_policy as $row) {
            $policy[$row->id] = $row->LeaveType;
        }
        $leave_year = $_GET['current_year'];
        $from_date = $leave_year.'-01-01';
        $to_date = $leave_year.'-12-31';
        $sql = 'SELECT LeavePolicyID,SUM(LeaveDays) AS LeaveDays FROM emp_leave_transaction WHERE EmployeeID="'.$emp_info['id'].'"
			    AND IsApproved=1 AND FromDate>="'.$from_date.'" AND ToDate<="'.$to_date.'" GROUP BY(LeavePolicyID)';
        //Mylog($sql);
        $enjoy_leave = $this->leave->query($sql)->fetchAll();
        $enjoy_cl=0;$enjoy_ml=0;$enjoy_el=0;
        if(!empty($enjoy_leave)){
            foreach($enjoy_leave as $row){ 
                if($policy[$row['LeavePolicyID']]=='CL'){
                    $enjoy_cl=$row['LeaveDays'];
                }
                if($policy[$row['LeavePolicyID']]=='SL'){
                    $enjoy_ml=$row['LeaveDays'];
                }
                if($policy[$row['LeavePolicyID']]=='EL'){
                    $enjoy_el=$row['LeaveDays'];
                }
                
            }
        }
        $sql = 'SELECT ald.LeavePolicyID,ald.LeaveDays
				FROM leave_policy_master lpm INNER JOIN allocated_leave_days ald ON ald.LeavePolicyID=lpm.id 
				WHERE ald.EmployeeID="'.$emp_info['id'].'"  GROUP BY(ald.LeavePolicyID)';
        //Mylog($sql);
        $allocated_leave = $this->leave->query($sql)->fetchAll();
        $allocated_leave_data = [];
        $balance_cl=0;$balance_ml=0;$balance_el=0;
        if(!empty($allocated_leave)){
            foreach($allocated_leave as $row){
                if($policy[$row['LeavePolicyID']]=='CL'){
                    $balance_cl=$row['LeaveDays']-$enjoy_cl;
                }
                if($policy[$row['LeavePolicyID']]=='SL'){
                    $balance_ml=$row['LeaveDays']-$enjoy_ml;
                }
                if($policy[$row['LeavePolicyID']]=='EL'){
                    $balance_el=$row['LeaveDays']-$enjoy_el;
                }
            }
        }
        //Mylog($allocated_leave_data);
        $leave_approved = $this->leave->table('emp_leave_transaction')->where('EmployeeID',$emp_info['id'])->where('IsApproved',1)->where('YEAR(FromDate)='.date_conversion('Y',$from_date))->fetchAll();
        $approved_data = '';        
        if(!empty($leave_approved)){$sl = 0;
            foreach($leave_approved as $row){ $cl=0;$ml=0;$el=0;
                if($policy[$row['LeavePolicyID']]=='CL'){$cl=$row['LeaveDays'];}
                if($policy[$row['LeavePolicyID']]=='SL'){$ml=$row['LeaveDays'];}
                if($policy[$row['LeavePolicyID']]=='EL'){$el=$row['LeaveDays'];}
                $approved_data .= '<tr>
                        <td>'.++$sl.'</td>
                        <td>'.date_conversion('d-m-Y',$row['FromDate']).'</td>
                        <td>'.date_conversion('d-m-Y',$row['ToDate']).'</td>
                        <td>'.$cl.'</td>
                        <td>'.$ml.'</td>
                        <td>'.$el.'</td>
                        <td></td>
        ...
        if(!empty($emp_info)){
            echo json_encode(array(
                ...
                'staff_category'=>$emp_info['staff_category'],
                'EmpType'=>$EmpType,
                'enjoy_cl'=>$enjoy_cl,
                'enjoy_ml'=>$enjoy_ml,
                'enjoy_el'=>$enjoy_el,
                ...
                'leave_history' => $approved_data
            ));
        }
    }
    public function multiple_apply_leave_abco()
    {
        $heading = "Leave";
        $title = "Multiple Apply Leave";
        $leave_types = $this->leave->table('leave_type')->fetchAll();
        ...
        return view('apply_leave/multiple_apply_leave_abco',compact('title','heading','leave_type'));        
    }
    public function multiple_leave_store_abco()
    {
        ...
        if($v->validate()) {
            ...
            //dd($LeaveType);
            for($i=0;$i<count($EmployeeID);$i++){
                if(!empty($EmployeeID[$i])){  
                    // $LeavePolicyID = $this->leave->table('leave_policy_master')->where('LeaveType',$LeaveType[$i])->fetch();
                    $LeavePolicyID = 'LV-16-1';
                    $is_leave_availed = $this->check_leave_availability($EmployeeID[$i],date_conversion('Y-m-d',$FromDate[$i]),date_conversion('Y-m-d',$ToDate[$i]),$LeaveDays[$i],$LeavePolicyID);
                    // dd($is_leave_availed);
                    if (!$is_leave_availed) {
            ...
            if($rs) {
                notification(['type'=>'success', 'message'=>'Apply Leave Success.']);
                return redirect('multiple_apply_leave/multiple_apply_leave_abco');
            }
            else {
                notification(['type'=>'success', 'message'=>'Failed.']);
                return redirect('multiple_apply_leave/multiple_apply_leave_abco');
            }
        } 
    }
     ...
     * @return View
     */
    public function show($leave_tran_id = null)
    {
        $heading = "Leave";
        $title = "Leave Approval";
        $leave_applied = $this->leave->table('emp_leave_transaction')->where('id', $leave_tran_id)->where('IsApproved', null)->fetch();
        $employee_info = $this->leave->table("employee_info")->where('id',$leave_applied->EmployeeID)->fetch();
        ...
        $employee_leaves = $this->leave->query("SELECT ei.id,ei.EmployeeCode,EmployeeName,PunchCardNo,lta.id as leave_tran_id FROM employee_info ei INNER JOIN emp_leave_transaction lta
    ...
    {
        //echo $id.'->'.$FromDate.'-->'.$ToDate.'-->'.$LeaveDays.'-->'.$LeavePolicyID;exit;
