...
<div class="content-wrapper">
    ...
    <section class="content pt-2 pb-0">
        <div class="container-fluid">
            ...
            <form action="<?php echo route("multiple_apply_leave/multiple_leave_store_abco"); ?>" class="role leave_form" method="post" id="form-id">
                <div class="row">
                    <div class="col-sm-2">
                    ...
                    <div class="col-sm-1 leave_days_part">
                        ...
                        <div class="form-group mb-2">
                            <div class="form-group mb-2">
                                ...
                                <?php echo form_input('ToDate[]', 'text', date('d-m-Y'), 'class="form-control form-control-sm  ToDate inputkey" autocomplete="off"'); ?>
                    ...
                    <div class="col-sm-2 mt-3">
                        ...
                        <h5 style="padding-top:100px;"><span id="leav_sapn"></span></h5>
                    </div>
                    <div class="col-sm-2 mt-3">
                        <table class="table table-sm table-bordered dataTableEmployeeInfo">
                            <thead>
                            ...
                            <tbody>
                                <tr>
                                    <td>CL</td>
                                    <td><span id="balance_cl" class="balance_cl"></span> </td>
                                </tr>
                                <tr>
                                    <td>ML</td>
                                    <td><span id="balance_ml" class="balance_ml"></span> </td>
                                </tr>
                                <tr>
                                    <td>EL</td>
                                    <td><span id="balance_el" class="balance_el"></span> </td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
...
<script>
    ...
    $(document).on('keypress', 'input,select', function(e) {
        if (e.which == 13) {
            ...
            if (index + 1 >= $canfocus.length) {
                $('.LeaveDays').focus();
                $('.LeaveDays').on('keypress', function(e) {
                    var key = e.which;
                    if (key == 13) {
                        $('.submitkey').click();
                    }
    ...
    $(document).ready(function() {
        ...
        $('.leave_days_part').on('keyup', '.LeaveDays', function() {
            if ($(this).val()) {
                ...
                var LeaveDaysHave = '',LeaveDaysHaveTxt='',CanTakeLeave='';
                
                if (!isNaN(from_date.getTime())) {
                    let LeaveDays = parseInt($(this).val(), 10);
                    if(type=='CL'){
                        LeaveDaysHaveTxt = $('#balance_cl').text();
                        LeaveDaysHave = LeaveDaysHaveTxt.replace('Days','');
                        CanTakeLeave = 3;
                    }else if(type=='SL'){
                        LeaveDaysHaveTxt = $('#balance_ml').text();
                        LeaveDaysHave = LeaveDaysHaveTxt.replace('Days','');
                        CanTakeLeave = 14;
                    }else if(type=='EL'){
                        LeaveDaysHaveTxt = $('#balance_el').text();
                        LeaveDaysHave = LeaveDaysHaveTxt.replace('Days','');
                        CanTakeLeave = 18;
                    }
                    //alert('LeaveDaysHave:'+LeaveDaysHave+'| CanTakeLeave:'+CanTakeLeave+'|LeaveDays:'+LeaveDays);
                    if(LeaveDaysHave>=LeaveDays){
                        if(LeaveDays<=CanTakeLeave || LeaveDays<0){
                            $("#leav_sapn").removeClass("badge badge-danger");
                            $("#leav_sapn").text("Ok! You can submit.").addClass("badge badge-success");
                            from_date.setDate(from_date.getDate() + LeaveDays - 1);
                            $(":submit").removeAttr("disabled");
                        }else{
                            $("#leav_sapn").removeClass("badge badge-success");
                            $("#leav_sapn").text("No. Of Days For "+type+" can't Be more than "+CanTakeLeave+" Days").addClass("badge badge-danger");
                            $(":submit").attr("disabled", true);
                        }
                    }else{
                        $("#leav_sapn").removeClass("badge badge-success");
                        $("#leav_sapn").text("Dont have enough Balance Leave").addClass("badge badge-danger");
                        $(":submit").attr("disabled", true);
                    }
